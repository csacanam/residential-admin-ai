...
if [ -f "$REPO_DIR/.env" ]; then
  ...
  echo "    OPENAI_API_KEY"
  echo "    KAPSO_API_KEY"
  echo "    KAPSO_PHONE_NUMBER_ID   ← Kapso → Settings → Phone Numbers → ID"
  echo "    KAPSO_TEMPLATE_NAME     ← nombre de la plantilla (ej: cobro_cartera)"
  echo "    KAPSO_TEMPLATE_ID       ← Kapso → Templates → ID numérico"
  echo "    AGENT_EMAIL"
